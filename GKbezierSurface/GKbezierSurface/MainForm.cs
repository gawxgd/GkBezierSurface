...
namespace GKbezierSurface
{
	public partial class MainForm : Form
	{
		...
		private float tetrahedronAlpha = 0;
		private float tetrahedronBeta = 0;

		private Timer tetrahedronTimer;
		public MainForm()
		{
			InitializeComponent();
		...
		private void TetrahedronAnimation()
		{
			tetrahedronTimer = new Timer();
			tetrahedronTimer.Interval = 100;
			tetrahedronTimer.Tick += TetrahedronAnimationTimer_Tick;
			tetrahedronTimer.Start();
		}

		private void TetrahedronAnimationTimer_Tick(object sender, EventArgs e)
		{
			tetrahedronAlpha += 0.05f; // Adjust step size for desired rotation sp
			tetrahedronBeta += 0.02f;

			if (tetrahedronAlpha >= 45) tetrahedronAlpha -= 45;
			if (tetrahedronBeta >= 10) tetrahedronBeta -= 10;

			// Apply rotation and redraw

			mesh.RotateTetrahedron(tetrahedronAlpha, tetrahedronBeta);
			var colorConfig = SetColorConfiguration();
			drawingHelper.Draw(mesh, drawTypeComboBox.SelectedIndex, colorConfig);
		}
